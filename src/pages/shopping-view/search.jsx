...
//   // Simple direct function for stopping recognition
//   function cancelVoiceSearch() {
//     if (recognition) {
//       try {
//         recognition.abort(); // Use abort() instead of stop() for more immediate effect
//       } catch (e) {
//         console.error("Error aborting recognition:", e);
//         try {
//           recognition.stop(); // Fallback to stop() if abort() fails
//         } catch (e2) {
...
//     // Always reset UI state regardless of recognition state
//     setIsListening(false);
//     setShowMicPopup(false);
//   }
...
//               {/* Simple cancel button - using standard button with onclick */}
//               <button
//                 className="w-full bg-gray-200 hover:bg-gray-300 active:bg-gray-400 text-gray-800 font-medium py-4 px-6 rounded-xl text-lg"
...
function SearchProducts() {
  ...
  // Update keyword when speech recognition transcript changes
  useEffect(() => {
    if (transcript) {
  ...
  const startVoiceSearch = () => {
    if (!browserSupportsSpeechRecognition) {
      toast({
        ...
        description: "Please use a supported browser like Chrome or Safari.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleAddToCart = async (productId, totalStock) => {
    ...
    try {
      await dispatch(addToCart({ userId: user.id, productId, quantity: 1 }));
